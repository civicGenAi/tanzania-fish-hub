...
CREATE POLICY "Customers can view own conversations"
  ...
  TO authenticated
  USING (customer_id = auth.uid() OR auth.is_admin());
...
CREATE POLICY "Users can update own messages"
  ...
  TO authenticated
  USING (sender_id = auth.uid() OR auth.is_admin());
...
COMMENT ON POLICY "Users can view own profile" ON profiles IS 'RLS policy allowing users to view their own profile';
COMMENT ON POLICY "Admins can view all profiles" ON profiles IS 'RLS policy allowing admins to view all profiles';
